...
| Response serialization/envelope conventions exist in an API/controller layer | `RESPONSE_KEYS.md` |
...
`docs/tasks/ADD_<TASK>.md` (UPPERCASE).
...
removed after `decisions.json` has copied its decisions, evidence, caps, root, and Git HEAD.
...
| `decisions.py init\|check <root>` | persists conditional-doc choices and makes unresolved, incomplete, or stale scan decisions fail verification |
...
1. **Gate existing state, then scan and harvest** — map the root rule/entry files first. If any
   ...
   first write and follow §Update Mode. Then run
   `python3 scripts/scan.py <root> --output <temporary-scan.json>`. For a custom loader, also pass
   `--entries <loader-entry>,AGENTS.md`. The scan manifest is the evidence for every
...
2. **Persist and select the doc set** — run
   `python3 scripts/decisions.py init <root> --scan <temporary-scan.json>`. Review the generated
...
3. **Write docs/*.md** — each rule cites real project evidence. Default form is the backticked
...
1. **Script** — `python3 scripts/verify_rules.py <project-root>`. When the project uses a different
   ...
   - **conditional-doc closure** — a current generation manifest requires
     ...
     docs, and source commits newer than the scan fail
...
- Refresh `.rule-architect/decisions.json` from the new scan, carrying forward any still-valid manual
  resolutions and reasons. Resolve every new `unknown` before writing conditional docs.
- A `--update` run MUST finish in the current output contract: real `AI_RULES.md` source plus two
  same-mode runtime entries that point directly to it. Do not merely report a legacy layout and leave it.
...
4. Replace both runtime entries with relative symlinks or portable pointers to `AI_RULES.md` in the
   ...
   the neutral index.
5. Record the complete migrated set with `manifest.py record --replace` only after enumerating every
   generated rule file. Re-run `manifest.py check`, then continue the ordinary diff update.

The migration changes ownership and entry shape, not rule meaning. Preserve the index body and linked
docs before applying separately justified rule additions, removals, or edits.
